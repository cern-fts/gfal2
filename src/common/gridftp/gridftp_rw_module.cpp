...
struct GridFTP_File_desc{
	...
	GridFTP_File_desc(GridFTP_stream_state * s, const std::string & _url, int flags) : stream(s){		
		gfal_print_verbose(GFAL_VERBOSE_TRACE, "create descriptor for %s", _url.c_str());		
	...
	virtual ~GridFTP_File_desc(){		
		gfal_print_verbose(GFAL_VERBOSE_TRACE, "destroy descriptor for %s", url.c_str());
	}	
...
inline int gridftp_rw_commit_put(const Glib::Quark & scope, GridFTP_File_desc* desc){
	...
	if(is_write_only(desc->open_flags)){
		gfal_print_verbose(GFAL_VERBOSE_TRACE," commit change for the current stream PUT ... ");	
		...
		gfal_print_verbose(GFAL_VERBOSE_TRACE," commited with success ... ");						
	}
	return 0;
}
