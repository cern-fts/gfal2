...
gfal_fdesc_container_handle gfal_file_handle_container_instance(
        gfal_descriptors_container* fdescs, GError** err)
{
    ...
    if (file_handle != NULL)
        ...
        file_handle = fdescs->file_container =
                gfal_file_descriptor_handle_create(NULL);
        if (!file_handle)
            gfal2_set_error(err, gfal2_get_plugins_quark(), EIO, __func__,
