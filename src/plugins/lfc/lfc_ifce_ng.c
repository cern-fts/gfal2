...
int _Cthread_addcid(char *, int, char *, int, Cth_pid_t *, unsigned, void *(*)(void *),
    int); // hack in order to use the internal CThread API ( DPNS limitation )
        ...
        if (tab_envar[i] == NULL) {
            switch (tab_type[i]) {
                case 0: {
                    ...
                    if (!tmp_err) {
                        gfal2_log(G_LOG_LEVEL_DEBUG, "lfc plugin : setup env var value %s to %s", tab_envar_name[i],
                            value);
                ...
                case 1: {
                    ...
                    if (!tmp_err) {
                        ...
                        gfal2_log(G_LOG_LEVEL_DEBUG, "lfc plugin : setup env var value %s to %d", tab_envar_name[i],
                            v2);
