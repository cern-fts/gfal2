...
int main(int argc, char** argv)
{
    ...
    g_assert(
            gfalt_get_create_parent_dir(params, &tmp_err) == TRUE && tmp_err == NULL);
    ...
    // begin copy
    printf(" begin to copyfile to root dir from %s to %s", src_uri,
            dst_uri_simple);
    if ((ret = gfalt_copy_file(handle, params, src_uri, dst_uri_simple,
            &tmp_err)) != 0) {
        printf(" error while the file transfer %d : %s.\n", tmp_err->code,
                tmp_err->message);
        return -1;
