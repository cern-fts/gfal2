...
IF(PLUGIN_FILE)
        test_del_nonex("FILE" "${file_prefix}")
        test_del("FILE" "${file_prefix}")
        test_mkdir_unlink("FILE" "${file_prefix}")
...
IF(PLUGIN_SRM)
        ...
        test_del_nonex("SRM_CASTOR" "${srm_prefix_castor}")
        ...
        test_mkdir_unlink("SRM_CASTOR"  "${srm_prefix_castor}")
        # stat tests
...
IF(PLUGIN_GRIDFTP)
        ...
        test_stat_all("GRIDFTP" ${gsiftp_prefix_dpm})
        test_access("GRIDFTP" ${gsiftp_prefix_dpm})
        test_checksum_simple("GRIDFTP_ADLER32" ${gsiftp_prefix_dpm} ADLER32)
        test_checksum_simple("GRIDFTP_MD5" ${gsiftp_prefix_dpm} MD5)
        test_checksum_simple("GRIDFTP_CRC32" ${gsiftp_prefix_dpm} CRC32)
        test_mkdir_all("GRIDFTP" ${gsiftp_prefix_dpm})
        test_chmod_all("GRIDFTP" ${gsiftp_prefix_dpm} 0565 060 0360 0767)
        test_rmdir_all("GRIDFTP" ${gsiftp_prefix_dpm})
        test_readdir_full("GRIDFTP" ${gsiftp_prefix_dpm})
        test_rwt_all("GRIDFTP" ${gsiftp_prefix_dpm} 4578)
        test_rwt_all("GRIDFTP_single" ${gsiftp_prefix_dpm} 1)
        test_rwt_seq("GRIDFTP" ${gsiftp_prefix_dpm} 100 4560)
        test_rwt_seq("GRIDFTP_unit" ${gsiftp_prefix_dpm} 1 10)
        test_rwt_seek("GRIDFTP" ${gsiftp_prefix_dpm} 100 4560)
        test_space("GRIDFTP" ${gsiftp_prefix_dpm})

        test_stat_all("FTP" ${ftp_prefix})
ENDIF(PLUGIN_GRIDFTP)

IF(PLUGIN_HTTP)
        test_del_nonex("DAVS_DPM" ${davs_prefix_dpm})
        test_del("DAVS_DPM" ${davs_prefix_dpm})
        test_mkdir_unlink("DAVS_DPM" ${davs_prefix_dpm})
        test_stat_all("DAVS" ${davs_prefix_dpm})
        test_checksum_simple("DAVS_ADLER32" ${davs_prefix_dpm} ADLER32)
        test_checksum_simple("DAVS_MD5"     ${davs_prefix_dpm} MD5)
        test_checksum_simple("DAVS_CRC32"   ${davs_prefix_dpm} CRC32)
        test_mkdir_all("DAVS" ${davs_prefix_dpm})
        test_rename("DAVS" ${davs_prefix_dpm})
        # chmod not supported
        test_rmdir_all("DAVS" ${davs_prefix_dpm})
        test_readdir_full("DAVS" ${davs_prefix_dpm})
...
IF(PLUGIN_XROOTD)
    test_del_nonex("XROOTD_DPM" ${root_prefix_dpm})
    test_del("XROOTD_DPM" ${root_prefix_dpm})
    test_mkdir_unlink("XROOTD_DPM" ${root_prefix_dpm})
